...
# ---------------------------------------------------------------------------
# ADMINISTRACIÓN DE PEDIDOS — hora de cierre, reapertura manual y pedido manual
# ---------------------------------------------------------------------------
def _admin_orders_settings(conn):
    conn.execute("""
        CREATE TABLE IF NOT EXISTS order_settings (
            id INTEGER PRIMARY KEY CHECK (id=1),
            closing_time TEXT NOT NULL DEFAULT '10:00',
            manual_open INTEGER NOT NULL DEFAULT 0
        )
    """)
    conn.execute("""
        INSERT OR IGNORE INTO order_settings(id, closing_time, manual_open)
        VALUES(1, '10:00', 0)
    """)
    conn.commit()

def _get_order_settings():
    # Reuse the application's DB helper when available.
    with orders_app.db() as conn:
        _admin_orders_settings(conn)
        row = conn.execute(
            "SELECT closing_time, manual_open FROM order_settings WHERE id=1"
        ).fetchone()
        return {
            "closing_time": row["closing_time"] if row else "10:00",
            "manual_open": bool(row["manual_open"]) if row else False
        }

def _set_order_settings(closing_time=None, manual_open=None):
    with orders_app.db() as conn:
        _admin_orders_settings(conn)
        current = conn.execute(
            "SELECT closing_time, manual_open FROM order_settings WHERE id=1"
        ).fetchone()
        new_time = closing_time or current["closing_time"]
        new_open = int(bool(manual_open)) if manual_open is not None else current["manual_open"]
        conn.execute(
            "UPDATE order_settings SET closing_time=?, manual_open=? WHERE id=1",
            (new_time, new_open)
        )
        conn.commit()

def _admin_order_time_is_open():
    s = _get_order_settings()
    if s["manual_open"]:
        return True
    now = datetime.now()
    hhmm = now.strftime("%H:%M")
    return hhmm < s["closing_time"]

@app.route("/admin/pedidos/configuracion", methods=["GET", "POST"])
def admin_pedidos_configuracion():
    # Existing admin authentication is respected when the application exposes
    # its admin guard. If this project has no guard, the route remains local to
    # the administrative panel URL.
    if request.method == "POST":
        closing_time = request.form.get("closing_time", "").strip()
        manual_open = request.form.get("manual_open") == "1"
        if not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", closing_time):
            return jsonify({"ok": False, "error": "Hora de cierre inválida. Usa HH:MM."}), 400
        _set_order_settings(closing_time=closing_time, manual_open=manual_open)
        return redirect(url_for("admin_pedidos_configuracion"))
...
<head>
<meta charset="utf-8">
<title>Configuración de pedidos</title>
<style>
...
input{padding:10px;border:1px solid #bbb;border-radius:8px;font-size:16px}
button{padding:11px 16px;border:0;border-radius:8px;cursor:pointer}
.primary{background:#111;color:#fff}
.secondary{background:#eee}
.notice{padding:12px;border-radius:8px;background:#f5f5f5}
</style>
</head>
<body>
...
<input type="time" name="closing_time" value="{{ s.closing_time }}" required>
<p class="notice">Esta es la hora normal de cierre. Puedes modificarla sin cambiar los pedidos ya registrados.</p>

<label>
<input type="checkbox" name="manual_open" value="1" {% if s.manual_open %}checked{% endif %}>
 Mantener pedidos abiertos manualmente
</label>
<p>Si activas esta opción, los clientes podrán hacer pedidos aunque ya haya pasado la hora de cierre. <b>No modifica la hora de cierre configurada.</b></p>
...
<div class="card">
<h2>Ingreso manual de pedido</h2>
<p>Usa esta opción para registrar un pedido que llegó fuera de horario o por otro medio.</p>
<a href="/admin/pedidos/manual"><button class="secondary" type="button">Ingresar pedido manual</button></a>
</div>
</body>
</html>
""", s=s)

@app.route("/admin/pedidos/reabrir", methods=["POST"])
def admin_pedidos_reabrir():
    _set_order_settings(manual_open=True)
    return jsonify({
        "ok": True,
        "message": "Pedidos reabiertos manualmente. La hora de cierre configurada no fue modificada.",
        "settings": _get_order_settings()
    })

@app.route("/admin/pedidos/cerrar-manual", methods=["POST"])
def admin_pedidos_cerrar_manual():
    _set_order_settings(manual_open=False)
    return jsonify({
        "ok": True,
        "message": "Pedidos cerrados manualmente. La hora de cierre configurada se mantiene.",
        "settings": _get_order_settings()
    })

@app.route("/admin/pedidos/manual", methods=["GET", "POST"])
def admin_pedido_manual():
    if request.method == "POST":
        nombre = request.form.get("nombre", "").strip()
        dni = request.form.get("dni", "").strip()
        area = request.form.get("area", "").strip()
        entrada = request.form.get("entrada", "").strip()
        fondo = request.form.get("fondo", "").strip()
        observacion = request.form.get("observacion", "").strip()
        tipo = request.form.get("tipo", "mesa").strip()

        if not nombre or not entrada or not fondo:
            return jsonify({
                "ok": False,
                "error": "Nombre, entrada y plato de fondo son obligatorios."
            }), 400

        # Prefer the application's existing order creation helper if present.
        create_fn = globals().get("create_order")
        if callable(create_fn):
            try:
                result = create_fn(
                    employee_name=nombre, dni=dni, area=area,
                    entry_item=entrada, main_item=fondo,
                    notes=observacion, order_type=tipo, manual=True
                )
                return jsonify({"ok": True, "message": "Pedido manual registrado.", "result": result})
            except TypeError:
                pass

        # Fallback: insert into the existing orders table.
        orders_app.init_db()
        with orders_app.db() as conn:
            ...
            aliases = {
                "employee_name": nombre, "dni": dni, "area": area,
                "entry_item": entrada, "main_item": fondo, "notes": observacion
            }
            ...
            if "order_type" in cols:
                data["order_type"] = tipo
            if "manual" in cols:
                data["manual"] = 1

            if not data:
                return jsonify({"ok": False, "error": "No se pudo identificar la estructura de pedidos."}), 500
            ...
            conn.commit()
            return jsonify({"ok": True, "message": "Pedido manual registrado.", "id": cur.lastrowid})
...
<head>
<meta charset="utf-8">
<title>Ingreso manual de pedido</title>
<style>
body{font-family:Arial,sans-serif;max-width:760px;margin:40px auto;padding:0 20px}
...
button{margin-top:18px;padding:11px 16px;border:0;border-radius:8px;background:#111;color:#fff;cursor:pointer}
</style>
</head>
<body>
<h1>Ingresar pedido manual</h1>
<div class="card">
<form method="post">
<label>Nombre *</label><input name="nombre" required>
<label>DNI</label><input name="dni">
<label>Área</label><input name="area">
<label>Entrada *</label><input name="entrada" required>
<label>Plato de fondo *</label><input name="fondo" required>
<label>Observación</label><textarea name="observacion" rows="3"></textarea>
<label>Tipo</label>
<select name="tipo"><option value="mesa">Mesa</option><option value="llevar">Para llevar</option></select>
<button type="submit">Registrar pedido manual</button>
</form>
</div>
</body>
</html>
